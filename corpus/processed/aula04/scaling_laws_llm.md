...
where S c ≈ 2 . 1 × 10 3 and αS ≈ 0 . 76, and Smin(S) is the minimum possible number of optimization steps (parameter updates) estimated using Equation (5.4).
...
- C ≈ 6NBS – an estimate of the total non-embedding training compute, where B is the batch size, and S is the number of training steps (ie parameter updates). We quote numerical values in PF-days, where one PF-day = 10 15 × 24 × 3600 = 8 . 64 × 10 19 floating point operations.
...
- Cm Cmin – an estimate of the minimum amount of non-embedding compute to reach a given value of the loss. This is the training compute that would be used if the model were trained at a batch size much less than the critical batch size.
...
Our choice of L(N, D) satisfies the first requirement because we can rescale Nc Nc , D c with changes in the vocabulary. This also implies that the values of Nc Nc , D c have no fundamental meaning.
...
Since the amount of data used by compute-efficient training grows slowly with the compute budget, the performance predicted by L(Cmin) eventually hits a lower bound set by the L(D) power law (see Figure 15). Let us work this out in more detail.
...
|---------------|---------|------------|-------------------------|------------------------------|
| N             | ∞       | ∞          | Fixed         | L(N) = (Nc Nc /N) αN         |
| ∞             | D       | Early Stop | Fixed         | L(D) = (D c /D) αD           |
| Optimal       | ∞       | C          | Fixed         | L(C) = (Cc Cc /C) αC (naive) |
| No Nopt       | D opt   | Cm Cmin    | B                       | L(Cmin) = C                  |
|               |         |            |                         |  c  min C                    |
|               |         |            |                         |  c  /Cmin α min C                              |
| N             | D       | Early Stop | Fixed         | L(N, D) =                                |
|               |         |            |                         |    Nc Nc N   α N α D  + D c D  αD                              |
| N             | ∞       | S steps    | B             | L(N, S) =   Nc Nc N  αN  +  S c S min (S,B)  αS                              |
...
|--------------------|---------------------------------------------|
| αN = 0 . 076       | Nc  Nc  = 8 . 8 × 10 13  params (non-embed) |
| αD = 0 . 095       | D c  = 5 . 4 × 10 13  tokens                |
| αC = 0 . 057       | Cc  Cc  = 1 . 6 × 10 7  PF-days             |
| α min C  = 0 . 050 | C                                           |
|                    |  c  min C                                   |
|                    |  c  = 3 . 1 × 10 8  PF-days                 |
| αB = 0 . 21        | B ∗  = 2 . 1 × 10 8 tokens                  |
| αS = 0 . 76        | S c  = 2 . 1 × 10 3 steps                   |
...
|--------------------------------------------------------------|--------------|----------------------------------|
| No Nopt  =  Ne  Ne  ·  C   | pN = 0 . 73  | Ne  Ne  = 1 . 3  ·  10 9  params |
|  m pN C                    |              |                                  |
|  min                       |              |                                  |
| B                                                            | pB = 0 . 24  | B e  = 2 . 0  ·  10 6 tokens     |
|  Bcrit  = B ∗ L 1/αB  =  B e C |              |                                  |
|  m pB C                    |              |                                  |
| S min  =  S e  ·  C        | pS = 0 . 03  | S e  = 5 . 4  ·  10 3 steps      |
|  m pS C                    |              |                                  |
|  min  (lower bound)        |              |                                  |
| D opt  =  D e  ·  C        | pD = 0 . 27  | D e  = 2  ·  10 10  tokens       |
|  m pD C                    |              |                                  |
|  min  (1 epoch)            |              |                                  |
...
which implies that for compute-efficient training, we should train to a fixed percentage αN αS ≈ 10% above the converged loss. Next, let's determine how the optimal loss depends on the compute budget. Eliminating N yields a power-law dependence of performance on compute:
...
Using A.6 and A.9, we can eliminate L in favor of Neff (L), the model size which reaches L most efficiently. From there, we find an expression for the excess compute needed as a consequence of using a suboptimal model size:
...
- We used the estimated training compute C ≈ 6NBS, which did not include contributions proportional to n ctx (see Section 2.1). So our scalings with compute may be confounded in practice in the regime of very large nctx, specifically where nctx &amp; 12dmodel .
...
|------------|------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|            | 2 Fits to L(N, D)  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . | 2 Fits to L(N, D)  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .                                                                                                                                                            | 11                                                                                                                                                                                                                                                              |
|            | 3 Fits to L(N, S)                                                                                    | 3 Fits to L(N, S)                                                                                                                                                                                                                                               | 14                                                                                                                                                                                                                                                              |
|            | 4 Key trend equations . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .  | 4 Key trend equations . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .                                                                                                                                                             | 20                                                                                                                                                                                                                                                              |
|            | 5 Key parameters to trend fits . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . | 5 Key parameters to trend fits . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .                                                                                                                                                            | 20                                                                                                                                                                                                                                                              |
|            | 6 Trends for compute-efficient training . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .  | 6 Trends for compute-efficient training . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .                                                                                                                                                             | 20                                                                                                                                                                                                                                                              |
...
|            |                                                                                                      | [GARD18] Guy Gur-Ari, Daniel A. Roberts, and Ethan Dyer. Gradient descent happens in a tiny subspace. 2018, arXiv:1812.04754. 18                                                                                                                                | [GARD18] Guy Gur-Ari, Daniel A. Roberts, and Ethan Dyer. Gradient descent happens in a tiny subspace. 2018, arXiv:1812.04754. 18                                                                                                                                |
